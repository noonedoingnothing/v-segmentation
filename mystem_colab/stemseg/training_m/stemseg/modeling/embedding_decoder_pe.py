...
class SqueezingExpandDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        q = k = self.pe(self.input_proj(embeddings), mask)
        embeddings_ = self.self_attn(q, k, value = self.input_proj(embeddings), attn_mask = mask, key_padding_mask = embeddings.weight)[0]
