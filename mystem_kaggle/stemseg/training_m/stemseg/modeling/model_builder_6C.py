...
class TrainingModel(nn.Module):
    ...
    def run_backbone(self, image_seqs):
        ...
        """
        #print('erooor')
        #print('image_seqs.num_frames',image_seqs.num_frames)
        #print('image_seqs.num_seqs',image_seqs.num_seqs)

        
        height, width = image_seqs.tensors.shape[-2:]
        images_tensor = image_seqs.tensors.view(image_seqs.num_seqs * image_seqs.num_frames, 6, height, width)
        ...
        import cv2
        for i in range(0,images_tensorz.size(0)):
          img = torch.permute(images_tensorz[i],(1,2,0)).detach().cpu().numpy()
          cv2.imwrite('/content/'+str(i)+'.png',img)
        print(ad)
        """
